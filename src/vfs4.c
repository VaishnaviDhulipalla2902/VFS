...
if (ismounted != 1) {
	...
	source = fopen(P3, "rb");
	if (source == NULL )
		return ADDFILE_SOURCE_NOT_FOUND;
	//added by self, no appropriate error code found.
	...
	file_d* fd = prepareFileDescriptor(P1, P2, type);
	 //creating file descriptor without full path
...
int vfscopyfile(char *P1, char *P2) {
	...
	if (duplicate == NULL ) {
		...
		if (strcmp(strerror(errno), "Success") != 0)
			return ADDFILE_CANNOT_WRITE_TO_DATAFILE;
		//return something if error while writing
		//printf("Open error: %s\n\n", strerror(errno));
		fclose(fp);
		globaldata->num_f_d_used = globaldata->num_f_d_used + 1;
